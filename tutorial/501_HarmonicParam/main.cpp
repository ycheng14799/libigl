...
int main(int argc, char *argv[])
{
  ...
  viewer.callback_key_pressed = 
    [&V,&V_uv](igl::opengl::glfw::Viewer& viewer, unsigned int key, int /*mod*/)
  {
    if(key == '3' || key == '2')
    {
      // Plot the 3D mesh or 2D UV coordinates
      const auto & V_display = key == '3' ? V : V_uv;
      viewer.core().set_vertices(V_display);
      viewer.core().align_camera_center(V_display,F);
